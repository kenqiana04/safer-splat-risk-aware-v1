...
def write_metrics(output_dir: Path, args: argparse.Namespace, summaries: list[dict[str, Any]]) -> None:
    ...
    payload = {
        ...
        "active_constraint_logging": {
            "scope": "lowest-h selected baseline constraints up to active_log_limit per step",
            "active_log_limit": args.active_log_limit,
            "near_critical_h": args.near_critical_h,
        },
        "summary": summaries,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Detailed per-step logging for reproduced SAFER-Splat baseline.")
    parser.add_argument("--scene", choices=sorted(SCENES), default="stonehenge")
    parser.add_argument("--methods", nargs="+", choices=METHODS, default=["safer_splat_filter"])
    ...
    parser.add_argument("--active-log-limit", type=int, default=50)
    parser.add_argument("--near-critical-h", type=float, default=0.001)
    ...
    output_dir = args.output_dir or Path(f"work/risk_aware_cbf/results/baseline_detailed_logging_{args.scene}")
    ...
    try:
        ...
        logger.log(f"active_constraints_logging=partial_lowest_h_selected limit={args.active_log_limit}")
