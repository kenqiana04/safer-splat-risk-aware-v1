...
METHODS = ("no_filter", "safer_splat_filter", "risk_aware_topk_v0")
...
DEBUG_FIELDS = [
    ...
    "risk_score_name",
    "baseline_active_constraints_count",
    "risk_aware_selected_count",
    "forced_low_h_count",
    "forced_near_count",
...
def write_metrics(output_dir: Path, args: argparse.Namespace, summaries: list[dict[str, Any]]) -> None:
    ...
    payload = {
        ...
        "risk_aware_topk": {
            "insertion_point": "reproduction wrapper after baseline CBF minimal constraints and before Clarabel QP solve",
            "topk": args.topk,
            "h_critical": args.h_critical,
            "near_distance_threshold": args.near_distance_threshold,
            "risk_score": args.risk_score,
            "hard_fallback": "keeps low-h, near-field, and heading-cone constraints; falls back to baseline on incomplete metadata",
        },
        "summary": summaries,
    }
...
def run_trial(
    *,
    ...
    elif method == "risk_aware_topk_v0":
        if risk_table is None:
            raise ValueError("risk_aware_topk_v0 requires a risk score table")
        cbf = RiskAwareTopKCBF(
            gsplat,
            dynamics,
            alpha,
            beta,
            ...
            min_required_constraints=args.min_required_constraints,
            heading_force_threshold=args.heading_force_threshold,
            heading_force_distance=args.heading_force_distance,
        )
    ...
    for step_idx in range(args.max_steps):
        ...
        for active_row in active_rows_for_step(
            ...
            near_critical_h=args.h_critical,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare no-filter, SAFER-Splat baseline, and risk-aware top-k V0 wrapper.")
    parser.add_argument("--scene", choices=sorted(SCENES), default="stonehenge")
    parser.add_argument("--methods", nargs="+", choices=METHODS, default=["no_filter", "safer_splat_filter", "risk_aware_topk_v0"])
    parser.add_argument("--trial-start", type=int, default=0)
    parser.add_argument("--trial-end", type=int, default=2)
    parser.add_argument("--max-steps", type=int, default=800)
    parser.add_argument("--topk", type=int, default=300)
    parser.add_argument("--h-critical", type=float, default=0.0006)
    ...
    parser.add_argument("--risk-score-table", type=Path, default=Path("work/risk_aware_cbf/results/risk_score_table_v0.csv"))
    parser.add_argument("--goal-tolerance", type=float, default=0.001)
    parser.add_argument("--min-required-constraints", type=int, default=1)
    parser.add_argument("--heading-force-threshold", type=float, default=0.65)
    parser.add_argument("--heading-force-distance", type=float, default=0.15)
    ...
    output_dir = args.output_dir or Path(f"work/risk_aware_cbf/results/risk_aware_topk_{args.scene}")
    ...
    debug_writer = CsvAppender(output_dir / "risk_aware_selection_debug.csv", DEBUG_FIELDS)
    ...
    try:
        ...
        logger.log(
            "risk_aware_topk="
            f"topk={args.topk} h_critical={args.h_critical} "
            f"near_distance_threshold={args.near_distance_threshold} risk_score={args.risk_score}"
        )
        ...
        risk_table = None
        if "risk_aware_topk_v0" in args.methods:
